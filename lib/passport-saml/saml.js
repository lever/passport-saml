...
SAML.prototype.generateAuthorizeRequest = function (req, isPassive, callback) {
  ...
  .then(function(){
    ...
    if (!self.options.disableRequestedAuthnContext) {
      ...
      var authnContext = self.options.authnContext;
      if (!Array.isArray(authnContext)) {
        authnContext = [authnContext];
      }

      authnContext.forEach(function(value) {
      ...
      if (authnContextClassRef.length === 1) {
        authnContextClassRef = authnContextClassRef[0];
      }

      request['samlp:AuthnRequest']['samlp:RequestedAuthnContext'] = {
        '@xmlns:samlp': 'urn:oasis:names:tc:SAML:2.0:protocol',
        '@Comparison': 'exact',
        'saml:AuthnContextClassRef': authnContextClassRef
      };
    }
...
SAML.prototype.validatePostResponse = function (container, callback) {
  ...
  function debugSignature() {
    // Log out the assertion and cert details so we can experiment with the values for ourselves
    console.warn(JSON.stringify({
      feature: 'passport-saml',
      message: 'Invalid SAML signature in validatePostResponse()',
      cert: self.options.cert,
      samlResponse: xml
    }))
    // Log out a stack trace so we know which path the code took to get here
    console.trace('Invalid SAML signature in validatePostResponse()')
  }
  ...
  .then(function() {
    ...
    if (assertions.length + encryptedAssertions.length > 1) {
      ...
      debugSignature()
      throw new Error('Invalid signature - multiple assertions provided');
    }
    ...
    if (assertions.length === 1) {
      if (self.options.cert &&
        ...
        debugSignature();
        throw new Error('Invalid signature - unencrypted assertion)');
      }
      ...
      return Q.ninvoke(xmlenc, 'decrypt', encryptedAssertionXml, xmlencOptions)
        .then(function(decryptedXml) {
          ...
          if (self.options.cert &&
            ...
            debugSignature();
            throw new Error('Invalid signature - encrypted assertion');
          }
...
SAML.prototype.checkTimestampsValidityError = function(nowMs, notBefore, notOnOrAfter) {
  ...
  if (notBefore) {
    ...
    if (nowMs + self.options.acceptedClockSkewMs < notBeforeMs)
        return new Error('SAML assertion not yet valid - now: ' + nowMs + ', notBefore: ' + notBeforeMs);
  }
  if (notOnOrAfter) {
    var notOnOrAfterMs = Date.parse(notOnOrAfter);
    if (nowMs - self.options.acceptedClockSkewMs >= notOnOrAfterMs)
      return new Error('SAML assertion expired - now: ' + nowMs + ', notOnOrAfter: ' + notOnOrAfterMs);
  }
...
SAML.prototype.validatePostRequest = function (container, callback) {
  ...
  parser.parseString(xml, function (err, doc) {
    ...
    if (self.options.cert && !self.validateSignature(xml, dom.documentElement, self.options.cert)) {
      console.warn(JSON.stringify({
        feature: 'passport-saml',
        message: 'Invalid SAML signature in validatePostRequest()',
        cert: self.options.cert,
        samlRequest: xml
      }))
